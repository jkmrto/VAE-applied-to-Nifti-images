...
from lib.aux_functionalities.functions import \
    get_batch_from_samples_supervised_3d
...
class CVAE():
    ...
    def __build_graph(self):
        ...
        self.in_flat_images = tf.placeholder(tf.float32, [None, self.total_size],
                                             "input_images")
        self.in_labels = tf.placeholder(tf.float32, [None, 2], "labels")
    ...
    def __generation(self, z, reuse_bool):
        with tf.variable_scope("generation", reuse=reuse_bool):
            z_develop = ops.dense(z, self.n_z + 2, self.input_dense_layer_dim[1],
                                  scope='z_matrix')
    ...
    def train(self, X, Y, n_iters=1000, batchsize=10, tempSGD_3dimages=False,
              iter_show_error=10, save_bool=False, suffix_files_generated=" ",
              iter_to_save=100, break_if_nan_error_value=True, max_to_keep=3):
        ...
        try:
            for iter in range(1, n_iters+1, 1):
                ...
                feed_dict = {self.in_flat_images: batch_flat,
                             self.in_labels:
                                 self.__generate_double_label(batch_labels)}
    ...
    def __generate_and_save_temp_3d_images(self,
            regen_batch, labels_batch, suffix):
        feed_dict = {self.in_flat_images: regen_batch,
                     self.in_labels: self.__generate_double_label(labels_batch)}
...
def auto_execute_with_session_folders():
    ...
    session_name = "test_over_cvae_labeled"
    path_to_session = \
