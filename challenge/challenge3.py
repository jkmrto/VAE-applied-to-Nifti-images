...
for i in range(len(trainidx)):
    ...
    z_train=np.zeros((len(trainidx[i]),DIM_LAT))   # Latent space
    k=0
    for j in trainidx:
        mus,_ = v.encode(train_data[j,:].reshape(1,DIM))
        z_train[k,:]=mus
        k=k+1
    z_test=np.zeros((len(testidx[i]),DIM_LAT))   # Latent space
    k=0
    for j in testidx:
        mus,_ = v.encode(train_data[j,:].reshape(1,DIM))
        z_test[k,:]=mus
    ...
    clf.fit(z_train, trainlabels[trainidx[i]]) 
    p=clf.predict(z_test)
    acc=np.sum(np.equal(p,trainlabels[testidx[i]]).astype(np.int32))/len(p)
    print('Acc=%1.4f'%acc)
