...
for train, test in skf.split(stack_train, trainlabels):
    ...
    for i in range(100):
        ...
        for trainint, testint in skfint.split(trset, labelsplit):
            sess.run(train_step, {X:trset[trainint,:,:], Y_: trlabel[trainint,:], lr: lrate, pkeep: 0.75})
            #sess.run(train_step, {X:np.fliplr(trset[testint,:,:]), Y_: trlabel[testint,:], lr: lrate, pkeep: 0.75})
        acc, c = sess.run([accuracy, cross_entropy], {X:trset[testint,:,:] , Y_: trlabel[testint,:], pkeep: 1.0})
        print(acc)
