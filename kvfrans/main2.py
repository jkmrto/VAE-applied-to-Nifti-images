...
class CNNVAE():
    ...
    def recognition(self, input_images):
        with tf.variable_scope("recognition",reuse=None):
    ...
    def generation(self, z):
        with tf.variable_scope("generation",reuse=None):
            ...
            h1 = tf.nn.relu(ops.conv2d_transpose(z_matrix, [self.batchsize, 14, 14, 16], "g_h1"))
            h2 = ops.conv2d_transpose(h1, [self.batchsize, 28, 28, 1], "g_h2")
            h2 = tf.nn.sigmoid(h2)
            ...
            for epoch in range(10):
                for idx in range(int(self.n_samples / self.batchsize)):
                    ...
                    if idx % (self.n_samples - 3) == 0:
                        ...
                        saver.save(sess, os.getcwd()+"/training",global_step=epoch)
